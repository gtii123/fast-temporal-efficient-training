...
if __name__ == '__main__':
    ...
    #                                         num_workers=args.workers, pin_memory=True)
    nmnist_train = NMNIST(root='./N-MNIST', train=True, data_type='frame', frames_number=10, split_by='number')
    nmnist_test = NMNIST(root='./N-MNIST', train=False, data_type='frame', frames_number=10, split_by='number')
    train_loader = DataLoader(dataset=nmnist_train, batch_size=64, shuffle=True, drop_last=False, num_workers=0)
    test_loader = DataLoader(dataset=nmnist_test, batch_size=64, shuffle=True, drop_last=False, num_workers=0)
    model =VGGSNNwoAP()
    ...
    optimizer = torch.optim.Adam(parallel_model.parameters(), lr=args.lr)
    ...
    for epoch in range(args.epochs):
        ...
        #     best_epoch = epoch + 1
        if epoch > 18:
            torch.save(parallel_model.state_dict(), 'VGGSNN_woAP.pth')
